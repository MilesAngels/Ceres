...
app.post('/register', async function (req, res) {
  try {
    const { user_name, user_password, user_fname, user_lname, admin_flag } = req.body;

    const isAdmin = admin_flag ? 1 : 0

    const hashedPassword = await bcrypt.hash(user_password, 10);

    const [user] = await req.db.query(
      `INSERT INTO user (user_name, user_password, user_fname, user_lname,admin_flag)
      VALUES (:username, :hashedPassword, :user_fname, :user_lname, :userIsAdmin);`,
      { user_name, hashedPassword, user_fname, user_lname, admin_flag: isAdmin });
